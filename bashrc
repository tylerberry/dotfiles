...
if [[ "${safe_term}" == "xterm.color" || "${safe_term}" == "xterm.xfree86" || "${safe_term}" == "screen.xterm.xfree86" ]] ; then
    use_color=true
elif [[ -f /etc/DIR_COLORS ]] ; then
...
vc()
{
  if [[ -d CVS ]] ; then
    ...
    echo "Directory does not appear to be under version control."
  fi
}
export -f vc
...
alias xia='ssh tyler@xia.thoughtlocker.net'
alias zen='ssh tyler@zen.server.us'
